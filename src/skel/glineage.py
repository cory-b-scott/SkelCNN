...
class RandomGraphLineage(GraphLineage):
    ...
    def __init__(self, sizes, pval=2):
        ...
        pl = [
            torch.tensor(np.random.randint(low=0,high=2,size=(sizes[i], sizes[i-1]))) for i in range(1,len(sizes))
        ]
        pl = [(item / item.sum(0)).to_sparse().coalesce() for item in pl]
...
class GridGraphLineage(GraphLineage):
    ...
    def __init__(self, sizes):
        gl = [
            .5*nxgraph_to_torch_sparse(nx.grid_graph(sizes[i])) for i in range(len(sizes))
        ]
        ...
        pl = [
            (
                torch.tensor(
                    np.kron(
                        np.kron(
                            np.eye(sizes[i-1][0]),.25*np.ones((int(sizes[i][0]/sizes[i-1][0]),1))
                        ),
                        np.kron(
                            np.eye(sizes[i-1][1]),.25*np.ones((int(sizes[i][1]/sizes[i-1][1]),1))
                        ),
                    )
                )
...
class ParametrizedGraphLineage(GraphLineage):
    ...
    def __init__(self, old_lineage, randomize=False, norm_G=False, norm_P=True):
        self.norm_G = norm_G
        self.norm_P = norm_P

        if randomize:
            self.g_orig_list = [torch.sparse_coo_tensor(item.indices(), .25 - .5*torch.rand(item.values().shape), size=item.shape).float().coalesce() for item in old_lineage.gl]
            self.p_orig_list = [torch.sparse_coo_tensor(item.indices(), .25 - .5*torch.rand(item.values().shape), size=item.shape).float().coalesce() for item in old_lineage.pl]
        else:
