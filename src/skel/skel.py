...
def torch_sparse_kron(A, B):
    ...
    return torch.sparse_coo_tensor(i, v, [A.shape[0]*B.shape[0], A.shape[1]*B.shape[1]],device=A.device,is_coalesced=True)

def kronsum(A,B):
    return torch.kron(A, torch.eye(*B.shape, device=B.device)) + \
           torch.kron(torch.eye(*A.shape, device=A.device), B)
...
def torch_sparse_eye(n1,n2,dev='cpu'):
    ...
    i = torch.stack([torch.arange(n),
                      torch.arange(n)])
    v = torch.ones(n)
    return torch.sparse_coo_tensor(i, v, [n1,n2],device=dev,is_coalesced=True)
...
def torch_dense_skeletal_cross_product_blocks(glist, hlist, slist, plist):
    blocks = []
    gsizes = [item.shape for item in glist]
    hsizes = [item.shape for item in hlist]
    for i in range(len(glist)):
        ...
        for j in range(len(hlist)):
            if i == j:
                brow.append(kron(glist[i], hlist[j]))
            elif i-j == -1:
                brow.append(kron(
                    slist[i].T,plist[j-1]
                ))
                ...
                brow.append(
                    torch.zeros(
                        (gsizes[i][0]*hsizes[i][0],gsizes[j][1]*hsizes[j][1]),
...
def torch_dense_skeletal_box_cross_product_blocks(glist, hlist, slist, plist):
    blocks = []
    gsizes = [item.shape for item in glist]
    hsizes = [item.shape for item in hlist]
    for i in range(len(glist)):
        ...
        for j in range(len(hlist)):
            if i == j:
                brow.append(kron(glist[i], hlist[j]) + kronsum(glist[i], hlist[j]))
            elif i-j == -1:
                brow.append(kron(
                    slist[i].T,plist[j-1]
                ))
